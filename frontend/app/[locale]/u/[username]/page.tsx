...
export default async function PublicUsernameProfilePage({ params }: PublicProfilePageProps) {
  const { locale, username } = await params;
  const profile = await fetchApi<PublicProfile | null>(`/api/profile/public/by-username/${query(username)}`, null);
      ...
      <PublicExperienceFrame>
        <main className="profileUnavailable">
          <section>
            ...
            <p>Ce membre n est pas disponible publiquement pour le moment.</p>
            <div>
  ...
  const [posts, media, documents] = await Promise.all([
    fetchApi<PublicProfilePost[]>(`/api/profile/posts?username=${query(username)}`, []),
    fetchApi<PublicProfileMedia>(`/api/profile/media?username=${query(username)}`, { photos: [], videos: [], projects: [] }),
    fetchApi<PublicProfileDocument[]>(`/api/profile/documents?username=${query(username)}`, []),
  ]);
